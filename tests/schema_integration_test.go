...
func TestSchema(t *testing.T) {
	schemaBackup := "../pkg/kfctl/schema/schema.json.bak"
	schemaPath := "../pkg/kfctl/schema/schema.json"
	...
	output := &bytes.Buffer{}
	cmd := exec.Command("../bin/mixins/kfctl/kfctl", "schema")
	cmd.Stdout = output
	cmd.Stderr = output

	err = cmd.Start()
	require.NoError(t, err, "failed to start the kfctl schema command")

	err = cmd.Wait()
	t.Log(output)
	require.NoError(t, err, "kfctl schema failed")
}
